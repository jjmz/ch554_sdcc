...
//          mDelaymS( 200 );                                                     // 由于USB设备刚插入尚未稳定,故等待USB设备数百毫秒,消除插拔抖动
				...
				if ( s == ERR_SUCCESS ){
					...
					if ( len ) {
						...
						for ( i = 0; i < len; i ++ ){
							...
							putchar('x');printhex2(RxBuffer[i]);putchar(' ');
						}
						//printf("\n");
						printlf();
					}
					...
					printstr("Mouse error ");printhex2(s);printlf();
				}
			}
			else {
		...
		if ( loc != 0xFFFF ){                                                 // 找到了,如果有两个KeyBoard如何处理?
			...
			printstr( "Query Keyboard @"); printhex4(loc); printlf();
			i = (uint8_t)( loc >> 8 );
			len = (uint8_t)loc;
			SelectHubPort( len );                                             // 选择操作指定的ROOT-HUB端口,设置当前USB速度以及被操作设备的USB地址
			endp = len ? DevOnHubPort[len-1].GpVar[0] : ThisUsbDev.GpVar[0];        // 中断端点的地址,位7用于同步标志位
			//printf("%02X  ",endp);
			printhex2(endp); printstr("  ");
			if ( endp & USB_ENDP_ADDR_MASK ){                                 // 端点有效
				s = USBHostTransact( USB_PID_IN << 4 | endp & 0x7F, endp & 0x80 ? bUH_R_TOG | bUH_T_TOG : 0, 0 );// CH554传输事务,获取数据,NAK不重试
				if ( s == ERR_SUCCESS ){
					endp ^= 0x80;                                             // 同步标志翻转
					...
					printstr("keyboard error ");printhex2(s);printlf();
				}
			}
			else{
		...
		if(TIN0 == 0){                                                          //P10为低，开始打印
			...
			if ( loc != 0xFFFF ){                                                  // 找到了,如果有两个打印机如何处理?
				...
				printstr( "Query Printer @");printhex4(loc);printlf();
				i = (uint8_t)( loc >> 8 );
				len = (uint8_t)loc;
				SelectHubPort( len );                                              // 选择操作指定的ROOT-HUB端口,设置当前USB速度以及被操作设备的USB地址
				...
				if ( endp & USB_ENDP_ADDR_MASK ){                                  // 端点有效
					...
					if ( s == ERR_SUCCESS ){
						...
						printstr("Printer error ");printhex2(s);printlf();
				}
			}
		}				
		...
		if ( loc != 0xFFFF ){                                                 // 找到了
			...
			printstr( "Query USB_DEV_CLASS_HID @");printhex4(loc);printlf();
			...
			{	
				...
				//printf("endp: %02X\n",(uint16_t)endp);
				printstr("endp: ");printhex2(endp);printlf();
		...
		if ( loc != 0xFFFF ){                                                 // 找到了	
			...
			if ( (endp & USB_ENDP_ADDR_MASK) != 0 )                           //端点有效
			{
				...
				if ( s == ERR_SUCCESS ){
					...
					printstr("transmit error ");printhex2(s);printlf();
				}					
			}					
			SetUsbSpeed( 1 );                                                // 默认为全速
		} 

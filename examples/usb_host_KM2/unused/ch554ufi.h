...
#define ERR_CH554_ERROR			0x81	/* CH554硬件错误,可能需要复位CH554 */
...
#endif
/* 代码2XH-3XH用于USB主机方式的通讯失败代码,由CH554子程序模仿CH375的返回 */
/* 代码1XH用于USB主机方式的操作状态代码,由CH554子程序模仿CH375的返回 */
...
#define DEF_SECTOR_SIZE			0x0200	/* 磁盘的扇区大小 */
#define CH554vSectorSizeH		2		/* 磁盘的扇区大小的高字节,低字节总是0x00,数值2代替0x200 */
...
typedef __xdata FAT_DIR_INFO *PX_FAT_DIR_INFO;
...
//	} Open;								/* CMD_FileOpen, 枚举文件, CH554vFileSize最高位为1则各调用xFileNameEnumer,为0则返回指定序号的文件名 */
	struct {
	...
	struct {
		...
		uint8_t	mLbaCount;
		uint8_t	mRemainCnt;
		__xdata uint8_t	*mDataBuffer;			/* 输入参数: 缓冲区起始地址, 返回: 缓冲区当前地址 */
		uint32_t	mLbaStart;
	...
	struct {
		...
		__xdata uint8_t	*mByteBuffer;			/* 输入参数: 指向存放读出数据块的缓冲区 */
		uint16_t	mActCnt;
	...
	struct {
		...
		uint8_t	mReserved[3];
		__xdata uint8_t	*mBuffer;				/* 输入参数: 指向子程序库的变量的备份缓冲区,长度不小于80个字节 */
	} SaveVariable;						/* CMD_SaveVariable, 备份/保存/恢复子程序库的变量 */
} CMD_PARAM;

typedef __idata CMD_PARAM CMD_PARAM_I;
...
#define DISK_BASE_BUF_LEN		512		/* 默认的磁盘数据缓冲区大小为512字节,为0则禁止在.H文件中定义缓冲区并由应用程序在pDISK_BASE_BUF中指定 */
#endif

/* 子程序库中提供的变量 */
extern	volatile uint8_t	CH554IntStatus;				/* CH554操作的中断状态 */
extern	volatile uint8_t	CH554DiskStatus;			/* 磁盘及文件状态 */
extern	__xdata uint8_t	CH554vDiskFat;				/* 逻辑盘的FAT标志:1=FAT12,2=FAT16,3=FAT32 */
extern	uint8_t	CH554vSecPerClus;			/* 逻辑盘的每簇扇区数 */
extern	__xdata uint32_t	CH554vStartLba;				/* 逻辑盘的起始绝对扇区号LBA */
extern	__xdata uint32_t	CH554vDiskRoot;				/* 对于FAT16盘为根目录占用扇区数,对于FAT32盘为根目录起始簇号 */
extern	__xdata uint32_t	CH554vDataStart;			/* 逻辑盘的数据区域的起始LBA */
extern	__xdata uint32_t	CH554vStartCluster;			/* 当前文件或者目录的起始簇号 */
extern	uint32_t	CH554vFileSize;				/* 当前文件的长度 */
extern	uint32_t	CH554vCurrentOffset;		/* 当前文件指针,当前读写位置的字节偏移 */
extern	__xdata uint32_t	CH554vFdtLba;				/* 当前FDT所在的LBA地址 */
extern	uint32_t	CH554vLbaCurrent;			/* 当前读写的磁盘起始LBA地址 */
extern	__xdata uint16_t	CH554vFdtOffset;			/* 当前FDT在扇区内的偏移地址 */
extern	__xdata uint8_t	CH554vCurrentLun;			/* 磁盘当前操作逻辑单元号 */
extern	__bit	CH554vSubClassIs6;			/* USB存储类设备的子类为6,0则非6 */
extern	__xdata uint8_t	*pDISK_BASE_BUF;		/* 指向外部RAM的磁盘数据缓冲区,缓冲区长度不小于CH554vSectorSizeH,由应用程序初始化 */
extern	__xdata uint8_t	*pDISK_FAT_BUF;		/* 指向外部RAM的磁盘FAT数据缓冲区,缓冲区长度不小于CH554vSectorSizeH,由应用程序初始化 */

extern	CMD_PARAM_I	mCmdParam;				/* 命令参数 */

extern	__xdata uint8_t	RxBuffer[ MAX_PACKET_SIZE ];  // IN, must even address
extern	__xdata uint8_t	TxBuffer[ MAX_PACKET_SIZE ];  // OUT, must even address
...
extern	__xdata uint8_t	DISK_BASE_BUF[ DISK_BASE_BUF_LEN ];	/* 外部RAM的磁盘数据缓冲区,缓冲区长度为一个扇区的长度 */
#endif
extern	uint8_t	CH554ReadSector( uint8_t SectCount, __xdata uint8_t *DataBuf );	/* 从磁盘读取多个扇区的数据到缓冲区中 */
#ifdef	EN_DISK_WRITE
extern	uint8_t	CH554WriteSector( uint8_t SectCount, __xdata uint8_t *DataBuf );	/* 将缓冲区中的多个扇区的数据块写入磁盘 */
#endif

extern	uint8_t	CH554DiskConnect( void );	/* 检查磁盘是否连接并更新磁盘状态 */
extern	void	xFileNameEnumer( void );	/* 调用外部定义的子程序,文件名枚举回调子程序 */

extern	uint8_t	CH554LibInit( void );		/* 初始化CH554程序库,操作成功返回0 */

/* 子程序库中提供的子程序 */
/* 下述子程序中, 文件操作子程序CH554File*和磁盘查询子程序CH554DiskQuery都可能会用到磁盘数据缓冲区pDISK_BASE_BUF,
   并且有可能在pDISK_BASE_BUF中保存了磁盘信息, 所以必须保证pDISK_BASE_BUF不被用于其它用途,
   如果RAM较少, 要将pDISK_BASE_BUF临时用于其它用途, 那么在临时用完后必须调用CH554DirtyBuffer清除磁盘缓冲区 */
extern	uint8_t	CH554GetVer( void );		/* 获取当前子程序库的版本号 */
extern	void	CH554DirtyBuffer( void );	/* 清除磁盘缓冲区 */
extern	uint8_t	CH554BulkOnlyCmd( __xdata uint8_t *DataBuf );	/* 执行基于BulkOnly协议的命令 */
extern	uint8_t	CH554DiskReady( void );		/* 查询磁盘是否准备好 */
extern	uint8_t	CH554AnalyzeError( uint8_t iMode );	/* USB操作失败分析CH554IntStatus返回错误状态 */
extern	uint8_t	CH554FileOpen( void );		/* 打开文件或者枚举文件 */
extern	uint8_t	CH554FileClose( void );		/* 关闭当前文件 */
#ifdef	EN_DISK_WRITE
extern	uint8_t	CH554FileErase( void );		/* 删除文件并关闭 */
extern	uint8_t	CH554FileCreate( void );	/* 新建文件并打开,如果文件已经存在则先删除后再新建 */
extern	uint8_t	CH554FileAlloc( void );		/* 根据文件长度调整为文件分配的磁盘空间 */
#endif
extern	uint8_t	CH554FileModify( void );	/* 查询或者修改当前文件的信息 */
extern	uint8_t	CH554FileQuery( void );		/* 查询当前文件的信息 */
extern	uint8_t	CH554FileLocate( void );	/* 移动当前文件指针 */
extern	uint8_t	CH554FileRead( void );		/* 从当前文件读取数据到指定缓冲区 */
#ifdef	EN_DISK_WRITE
extern	uint8_t	CH554FileWrite( void );		/* 向当前文件写入指定缓冲区的数据 */
#endif
extern	uint8_t	CH554ByteLocate( void );	/* 以字节为单位移动当前文件指针 */
extern	uint8_t	CH554ByteRead( void );		/* 以字节为单位从当前位置读取数据块 */
#ifdef	EN_DISK_WRITE
extern	uint8_t	CH554ByteWrite( void );		/* 以字节为单位向当前位置写入数据块 */
#endif
extern	uint8_t	CH554DiskQuery( void );		/* 查询磁盘信息 */
extern	void	CH554SaveVariable( void );	/* 备份/保存/恢复子程序库的变量,用于子程序库在多个芯片或者U盘之间进行切换 */

extern	void	mDelayuS( uint16_t n );		// 以uS为单位延时
extern	void	mDelaymS( uint16_t n );		// 以mS为单位延时
extern	uint8_t	USBHostTransact( uint8_t endp_pid, uint8_t tog, uint16_t timeout );	// CH554传输事务,输入目的端点地址/PID令牌,同步标志,NAK重试时间,返回0成功,超时/出错重试
extern	uint8_t	HostCtrlTransfer( __xdata uint8_t *DataBuf, uint8_t *RetLen );	// 执行控制传输,8字节请求码在pSetupReq中,DataBuf为可选的收发缓冲区,实际收发长度返回在ReqLen指向的变量中
extern	void	CopySetupReqPkg( __code uint8_t *pReqPkt );  // 复制控制传输的请求包
//extern	uint8_t	CtrlGetDeviceDescr( void );  // 获取设备描述符,返回在TxBuffer中
extern	uint8_t	CtrlGetConfigDescr( void );  // 获取配置描述符,返回在TxBuffer中

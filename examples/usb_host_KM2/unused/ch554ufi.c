...
__xdata __at (0x0080) uint8_t	DISK_BASE_BUF[ DISK_BASE_BUF_LEN ];	/* 外部RAM的磁盘数据缓冲区,缓冲区长度为一个扇区的长度 */
#endif
...
uint8_t	CH554ReadSector( uint8_t SectCount, __xdata uint8_t *DataBuf )  /* 从磁盘读取多个扇区的数据到缓冲区中 */
{
	uint8_t	retry;
//	if ( use_external_interface ) return( extReadSector( CH554vLbaCurrent, SectCount, DataBuf ) );  /* 外部接口 */
	for( retry = 0; retry < 3; retry ++ ) {  /* 错误重试 */
		...
		pCBW -> mCBW_CB_Buf[ 2 ] = *(Puint8_t)&CH554vLbaCurrent;
		pCBW -> mCBW_CB_Buf[ 3 ] = *( (Puint8_t)&CH554vLbaCurrent + 1 );
		pCBW -> mCBW_CB_Buf[ 4 ] = *( (Puint8_t)&CH554vLbaCurrent + 2 );
		pCBW -> mCBW_CB_Buf[ 5 ] = *( (Puint8_t)&CH554vLbaCurrent + 3 );
...
#ifdef	EN_DISK_WRITE
uint8_t	CH554WriteSector( uint8_t SectCount, P__xdata uint8_t DataBuf )  /* 将缓冲区中的多个扇区的数据块写入磁盘 */
{
	uint8_t	retry;
...
/* 如果指定枚举序号CH554vFileSize为0xFFFFFFFF后调用FileOpen，那么每搜索到一个文件FileOpen都会调用本回调程序，
   ...
   如果在本回调程序xFileNameEnumer中将CH554vFileSize置为1，那么可以通知FileOpen提前结束搜索。以下是回调程序例子 */
	...
	FileCount = (uint16_t)( 0xFFFFFFFF - CH554vFileSize );  /* 当前文件名的枚举序号,CH554vFileSize初值是0xFFFFFFFF,找到文件名后递减 */
...
uint8_t	CH554LibInit( void )  /* 初始化CH554程序库,操作成功返回0 */
{
